...
class inter_att(nn.Module):
    ...
    def forward(self, embeds):
        ...
        print("First-order neighbor weight:", beta.data.cpu().numpy())
        z_lo = 0
        for i in range(len(embeds)):
            z_lo += embeds[i] * beta[i]
        return z_lo

#Intra-category attention
...
#Low order view encoder
class Lo_encoder(nn.Module):
    def __init__(self, hidden_dim, sample_rate, nei_num, attn_drop):
        super(Lo_encoder, self).__init__()
        ...
        for i in range(self.nei_num):
            ...
            for per_node_nei in nei_index[i]:
                if len(per_node_nei) >= sample_num:
                    select_one = torch.tensor(np.random.choice(per_node_nei, sample_num, replace=False))[
                        np.newaxis]
                else:
                    select_one = torch.tensor(np.random.choice(per_node_nei, sample_num, replace=True))[
                        np.newaxis]

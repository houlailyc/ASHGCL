...
#Calculate the similarity matrix of node attributes
def generate_similarity_matrix(feat_p, threshold):

    feat_p_np = feat_p.numpy()

    similarity_matrix = cosine_similarity(feat_p_np)
    similarity_matrix[similarity_matrix < threshold] = 0
    similarity_matrix[similarity_matrix >= threshold] = 1

    sparse_similarity = sp.csr_matrix(similarity_matrix)
    # Gets the index and value of a non-zero element
    coo = sparse_similarity.tocoo()
    indices = th.LongTensor([coo.row, coo.col])
    values = th.FloatTensor(coo.data)
    shape = th.Size(coo.shape)
...
def load_acm(ratio, type_num):
    ...
    #Positive sample
    pos = sp.load_npz(path + "combined_pos.npz")
...
def load_dblp(ratio, type_num):
    ...
    val = [th.LongTensor(i) for i in val]
    test = [th.LongTensor(i) for i in test]
    return [nei_p], [feat_a, feat_p], [apa, apcpa], pos, label, train, val, test

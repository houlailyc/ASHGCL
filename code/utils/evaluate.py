...
def evaluate(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd, isTest=True):
    hid_units = embeds.shape[1]
    xent = nn.CrossEntropyLoss()

    train_embs = embeds[idx_train]  # Gets the training set embedding vector
    val_embs = embeds[idx_val]
    test_embs = embeds[idx_test]

    train_lbls = torch.argmax(label[idx_train], dim=-1)# Get the training set tag
    ...
    for _ in range(20):
        ...
        for iter_ in range(200):
            ...
            logits = log(train_embs)  # train logits
            ...
            # val
            logits = log(val_embs)  # val logits
            preds = torch.argmax(logits, dim=1)  # Get predicted value
            ...
            # test
            logits = log(test_embs)  # test logits
